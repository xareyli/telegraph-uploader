...
class _HtmlToNodesParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs_list):
        if tag not in ALLOWED_TAGS:
            raise NotAllowedTag('{} tag is not allowed'.format(tag))
    ...
    def handle_endtag(self, tag):
        ...
        if not len(self.parent_nodes):
            raise InvalidHTML('{} missing start tag'.format(tag))
        ...
        if last_node['tag'] != tag:
            raise InvalidHTML('{} tag closed instead of {}'.format(tag, last_node["tag"]))
